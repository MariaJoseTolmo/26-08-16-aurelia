...
export class InspectionHistoryService {
  ...
  async getHistoryKpis(scope: InspectionDataScope = {}): Promise<InspectionHistoryKpisResponse> {
    ...
    const legacyInspectionIds = new Set(legacyImports.map((legacyImport) => legacyImport.inspectionId));
    ...
    const currentFindings = findings.filter((finding) =>
      ...
      && !legacyInspectionIds.has(finding.inspectionId)
      && finding.status !== InspectionFindingStatus.CANCELLED,
    );
    const currentClosedFindings = currentFindings.filter(
      (finding) => finding.status === InspectionFindingStatus.CLOSED,
    ).length;
    const closedLegacyInspections = closedInspections.filter((inspection) => legacyInspectionIds.has(inspection.id));
    const legacyFindingsCount = closedLegacyInspections.reduce(
      (total, inspection) => total + Math.max(0, inspection.findingsCount),
      0,
    );
    const legacyClosedFindingsCount = closedLegacyInspections.reduce(
      (total, inspection) => total + Math.max(0, inspection.findingsCount - inspection.openFindingsCount),
      0,
    );
    const totalFindingsCount = currentFindings.length + legacyFindingsCount;
    const totalClosedFindingsCount = currentClosedFindings + legacyClosedFindingsCount;
    ...
    return {
      ...
      averageClosureDays,
      closedFindingsRate: totalFindingsCount > 0
        ? Number(((totalClosedFindingsCount / totalFindingsCount) * 100).toFixed(2))
        : 0,
      contractorCompanies,
    };
  }
  ...
  async getHistoryTable(
    ...
    const [allInspections, allFindings, legacyImports, companies, areas, sectors, users, inspectionTypes] = await Promise.all([
      this.inspections.find(),
      this.findings.find(),
      this.legacyImports.find(),
      this.companies.find(),
    ...
    const rows = closedInspections.map((inspection, index) => {
      ...
      const isLegacy = legacyInspectionIds.has(inspection.id);
      const observations = isLegacy
        ? this.createLegacyObservationSummary(inspection)
        : this.createObservationSummary(inspectionFindings);
      const observationsCount = isLegacy
        ? Math.max(0, inspection.findingsCount)
  ...
  private createLegacyObservationSummary(
    ...
    const open = Math.min(total, Math.max(0, inspection.openFindingsCount));
    return {
      executed: 0,
      open,
