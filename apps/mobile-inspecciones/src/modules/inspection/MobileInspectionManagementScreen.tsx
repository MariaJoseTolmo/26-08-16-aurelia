...
function TypeChip({ value }: { value: string }) {
  ...
  return (
    <View style={[styles.typeChip, checklist ? styles.typeChipChecklist : styles.typeChipFinding]}>
      ...
      <Text style={[styles.typeChipText, { color: checklist ? '#006153' : '#0d3862' }]}>{checklist ? 'Checklist' : 'Hallazgo'}</Text>
    </View>
  );
}
...
function ManagementCard({ row, onPress }: { row: InspectionManagementTableRowResponse; onPress: () => void }) {
  const palette = tonePalette[statusTone(row)];
  ...
  return (
    <TouchableOpacity style={styles.card} activeOpacity={0.82} onPress={onPress}>
      ...
      <View style={styles.cardBody}>
        ...
        <View style={styles.cardMeta}><FontAwesome5 name="map-marker-alt" size={10} color={colors.placeholder} /><Text style={styles.cardMetaText}>{row.daysOpen} días</Text></View>
        <View style={styles.cardCounters}>{rows.length > 0 ? rows.map((counter) => <CounterRow key={counter.key} counter={counter} />) : <View style={styles.counterRow}><Text style={styles.counterEmpty}>Sin observaciones</Text></View>}</View>
...
function HistoryCard({ row, onPress }: { row: InspectionManagementTableRowResponse; onPress: () => void }) {
  const { area, sector } = splitAreaSector(row.areaSector);
  const observationCount = row.observations.closed || row.observationsCount;
  return (
    <TouchableOpacity style={styles.historyCard} activeOpacity={0.82} onPress={onPress}>
      <View style={styles.historyCardLine} />
      <View style={styles.historyCardBody}>
        <View style={styles.cardTop}>
          <Text style={styles.cardId}>{row.inspectionNumber.startsWith('#') ? row.inspectionNumber : `#${row.inspectionNumber}`}</Text>
          <TypeChip value={row.type} />
        </View>
        <Text style={styles.historyCardTitle}>{area} · {row.company}</Text>
        <View style={styles.historyMeta}><FontAwesome5 name="map-marker-alt" size={9} color="#acacac" /><Text style={styles.historyMetaText}>{sector} · {row.daysOpen} días</Text></View>
        <View style={styles.historyObservationBar}>
          <FontAwesome5 name="check-circle" size={10} color="#2a5c16" solid />
          <Text style={styles.historyObservationText}>{observationCount} {observationCount === 1 ? 'observación' : 'observaciones'}</Text>
        </View>
      </View>
    </TouchableOpacity>
  );
...
function activeFilterLabels(filters: MobileInspectionManagementFilters): string[] {
  const observationLabel: Record<string, string> = { executed: 'Ejecutadas', open: 'Abiertas', closed: 'Cerradas', rejected: 'Rechazadas' };
  return [
    filters.id ? `N° ${filters.id.startsWith('#') ? filters.id : `#${filters.id}`}` : '',
    filters.date ? `Fecha ${filters.date}` : '', filters.inspector ? `Inspector ${filters.inspector}` : '',
    filters.area ? `Área ${filters.area}` : '', filters.company ? `Empresa ${filters.company}` : '',
    filters.type ? `Tipo ${filters.type}` : '', filters.urgency ? `Urgencia ${filters.urgency}` : '',
    filters.count ? `N° obs. ${filters.count}` : '', filters.obs ? `Obs. ${observationLabel[filters.obs] ?? filters.obs}` : '',
    filters.daysMin ? `Días mín. ${filters.daysMin}` : '', filters.daysMax ? `Días máx. ${filters.daysMax}` : '',
...
export function MobileInspectionManagementScreen() {
  ...
  useEffect(() => { if (deepInspectionId) setSelectedInspectionId(deepInspectionId); }, [deepInspectionId]);
  useEffect(() => { if (deepMode === 'history') setMode('history'); else if (deepMode === 'management') setMode('management'); }, [deepMode]);

  const metrics = useMemo(() => {
    if (mode === 'history') {
      const kpis = data.historyKpis.data;
      return [
        { value: String(kpis?.closedInspections ?? '—'), label: 'Inspecciones cerradas', color: '#2a5c16' },
        { value: String(kpis?.averageClosureDays ?? '—'), label: 'Promedio de cierre', color: '#131313' },
      ];
    }
  ...
  async function refresh() { await Promise.all([data.table.refetch(), mode === 'management' ? data.managementKpis.refetch() : data.historyKpis.refetch(), draftsQuery.refetch()]); }
  function changeMode(nextMode: MobileInspectionManagementMode) { setMode(nextMode); setFilters(emptyFilters); }
  async function resumeDraft(id: string) {
    if (!capabilities.create) { Alert.alert('Sin permiso', 'Tu perfil no tiene permiso para continuar inspecciones.'); return; }
    const persisted = await getManualInspectionDraftById(id); if (!persisted) return;
    hydrateDraft(persisted.draft);
    if ((persisted.draftMode ?? 'manual') === 'chat') { router.push('/inspection/chat'); return; }
  ...
  function newInspection() { if (!capabilities.create) { Alert.alert('Sin permiso', 'Tu perfil no tiene permiso para crear inspecciones.'); return; } router.push('/inspection/start'); }
  function closeDetail() { setSelectedInspectionId(null); if (deepInspectionId || deepFindingId || deepGroup || deepMode || deepNotificationId) router.replace('/inspection/dashboard'); }

  if (!capabilities.read) return <SafeAreaView style={styles.safe}><View style={styles.denied}><FontAwesome5 name="lock" size={28} color={colors.gold} /><Text style={styles.deniedTitle}>Acceso restringido</Text><Text style={styles.deniedText}>Tu perfil no cuenta con inspections:read para consultar Gestión de Inspecciones.</Text></View></SafeAreaView>;

  return (
    <SafeAreaView style={styles.safe}>
      <View style={styles.screen}>
        <View style={styles.header}><HeaderGradient /><View style={styles.brandRow}><LogoMobile width={137} height={45} /><TouchableOpacity style={styles.bell} onPress={() => router.push('/inspection/notifications')}><BellIcon width={20} height={16} />{unreadNotifications > 0 ? <View style={styles.bellBadge}><Text style={styles.bellBadgeText}>{unreadNotifications > 99 ? '99+' : unreadNotifications}</Text></View> : null}</TouchableOpacity></View><Text style={styles.hello}>Hola,</Text><Text style={styles.name}>{user?.fullName ?? 'Usuario AurelIA'}</Text><View style={styles.role}><ShieldIcon width={13} height={10} /><Text style={styles.roleText}>{badge}</Text></View></View>
        ...
        <View style={styles.actions}><TouchableOpacity style={styles.filter} onPress={() => setFiltersVisible(true)}><FilterIcon width={15} height={12} /><Text style={styles.filterText}>Filtrar</Text>{filterCount > 0 ? <View style={styles.filterCount}><Text style={styles.filterCountText}>{filterCount}</Text></View> : null}</TouchableOpacity>{mode === 'management' && capabilities.create ? <TouchableOpacity style={styles.newButton} onPress={newInspection}><PlusIcon width={19} height={15} /><Text style={styles.newText}> Nueva inspección</Text></TouchableOpacity> : null}{activeLabels.length > 0 ? <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.activeFilters}>{activeLabels.map((label) => <View key={label} style={styles.activeFilter}><Text style={styles.activeFilterText}>{label}</Text></View>)}</ScrollView> : null}</View>
        <ScrollView style={styles.list} contentContainerStyle={styles.listContent} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { void refresh(); }} tintColor={colors.gold} />} showsVerticalScrollIndicator={false}>
          ...
          {!loading ? rows.map((row) => mode === 'history' ? <HistoryCard key={row.inspectionId} row={row} onPress={() => setSelectedInspectionId(row.inspectionId)} /> : <ManagementCard key={row.inspectionId} row={row} onPress={() => setSelectedInspectionId(row.inspectionId)} />) : null}
        ...
        <View style={styles.tabs}><FooterGradient /><TouchableOpacity style={[styles.tab, mode === 'management' && styles.tabSelected]} onPress={() => changeMode('management')}><View style={styles.tabCount}><Text style={styles.tabCountText}>{data.managementKpis.data?.openInspections ?? 0}</Text></View><Text style={[styles.tabText, mode === 'management' && styles.tabTextSelected]}>Gestión de inspecciones</Text>{mode === 'management' ? <View style={styles.tabLine} /> : null}</TouchableOpacity><TouchableOpacity style={[styles.tab, mode === 'history' && styles.tabSelected]} onPress={() => changeMode('history')}><View style={styles.tabDot}><View style={styles.tabDotInner} /></View><Text style={[styles.tabText, mode === 'history' && styles.tabTextSelected]}>Historial</Text>{mode === 'history' ? <View style={styles.tabLine} /> : null}</TouchableOpacity></View>
      </View>
...
const styles = StyleSheet.create({
  ...
  metricLabel: { marginTop: 2, color: colors.muted, fontSize: 9, lineHeight: 11, textAlign: 'center' },
  ...
  activeFilters: { gap: 6, paddingRight: 12 }, activeFilter: { borderRadius: 5, borderWidth: 1, borderColor: '#b4d1ed', backgroundColor: '#e6f3ff', paddingHorizontal: 8, paddingVertical: 4 }, activeFilterText: { color: '#0d3862', fontSize: 9, fontWeight: fontWeight.semibold },
  list: { flex: 1 }, listContent: { paddingHorizontal: 14, paddingVertical: 12, gap: 10 },
  ...
  cardLine: { height: 3 }, cardBody: { paddingHorizontal: 14, paddingTop: 12, paddingBottom: 13 }, cardTop: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 8 }, cardId: { color: '#24588b', fontSize: 12, fontWeight: fontWeight.bold }, cardChips: { flexDirection: 'row', alignItems: 'center', gap: 7, flexShrink: 1 },
  typeChip: { minHeight: 18, borderRadius: 6, flexDirection: 'row', alignItems: 'center', gap: 4, paddingHorizontal: 8, paddingVertical: 2 }, typeChipChecklist: { backgroundColor: '#c5fff6' }, typeChipFinding: { backgroundColor: '#e6f3ff' }, typeChipText: { fontSize: 10, fontWeight: fontWeight.bold },
  statusChip: { minHeight: 18, borderRadius: 6, flexDirection: 'row', alignItems: 'center', gap: 4, paddingHorizontal: 8, paddingVertical: 3, flexShrink: 1 }, statusChipText: { fontSize: 9, fontWeight: fontWeight.bold, flexShrink: 1 }, cardTitle: { marginTop: 7, color: colors.primary, fontSize: 13, lineHeight: 17, fontWeight: fontWeight.bold }, cardMeta: { marginTop: 4, flexDirection: 'row', alignItems: 'center', gap: 5 }, cardMetaText: { color: colors.muted, fontSize: 10.5 },
  cardCounters: { marginTop: 10, gap: 5 }, counterRow: { minHeight: 25, borderRadius: 7, backgroundColor: '#f7f7f7', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 8, paddingVertical: 5 }, counterLeft: { flexDirection: 'row', alignItems: 'center', gap: 5 }, counterLabel: { fontSize: 11, fontWeight: fontWeight.bold }, counterEmpty: { color: colors.muted, fontSize: 10.5 }, cardProgress: { marginTop: 9, flexDirection: 'row', alignItems: 'center', gap: 7 }, cardProgressRail: { flex: 1, height: 4, borderRadius: 2, backgroundColor: colors.border }, cardProgressFill: { height: 4, borderRadius: 2, backgroundColor: colors.gold }, cardProgressText: { minWidth: 28, color: colors.muted, fontSize: 9, fontWeight: fontWeight.bold, textAlign: 'right' },
  historyCard: { borderRadius: 12, borderWidth: 1.5, borderColor: colors.border, backgroundColor: colors.white, overflow: 'hidden', shadowColor: '#000', shadowOpacity: 0.06, shadowRadius: 3, shadowOffset: { width: 0, height: 1 }, elevation: 1 }, historyCardLine: { height: 3, backgroundColor: '#2a5c16' }, historyCardBody: { paddingHorizontal: 14, paddingTop: 12, paddingBottom: 14 }, historyCardTitle: { marginTop: 6, color: '#131313', fontSize: 13, lineHeight: 16, fontWeight: fontWeight.bold }, historyMeta: { marginTop: 3, flexDirection: 'row', alignItems: 'center', gap: 5 }, historyMetaText: { color: '#646464', fontSize: 11, lineHeight: 14 }, historyObservationBar: { marginTop: 10, minHeight: 24, borderRadius: 7, backgroundColor: '#e0ffd3', flexDirection: 'row', alignItems: 'center', gap: 6, paddingHorizontal: 8, paddingVertical: 5 }, historyObservationText: { color: '#2a5c16', fontSize: 11, fontWeight: fontWeight.semibold },
  drafts: { backgroundColor: colors.white, borderRadius: 12, borderWidth: 1, borderColor: colors.border, overflow: 'hidden' }, draftHeader: { padding: 14, flexDirection: 'row', justifyContent: 'space-between' }, draftTitle: { color: colors.primary, fontSize: 15, fontWeight: fontWeight.bold }, draftSub: { marginTop: 2, color: colors.muted, fontSize: 11 }, redDot: { width: 16, height: 16, borderRadius: 8, backgroundColor: '#c4365a' }, draftBody: { borderTopWidth: 1.5, borderTopColor: colors.border, flexDirection: 'row', alignItems: 'center', minHeight: 88, paddingHorizontal: 14, paddingVertical: 14, gap: 12 }, draftIcon: { width: 44, height: 44, borderRadius: 10, backgroundColor: '#ffeab8', alignItems: 'center', justifyContent: 'center' }, draftCopy: { flex: 1 }, draftName: { color: colors.primary, fontSize: 13, fontWeight: fontWeight.bold }, draftMeta: { marginTop: 2, color: colors.muted, fontSize: 11 }, progressWrap: { marginTop: 6, flexDirection: 'row', alignItems: 'center', gap: 8 }, progressRail: { flex: 1, height: 4, borderRadius: 2, backgroundColor: colors.border }, progressFill: { height: 4, borderRadius: 2, backgroundColor: colors.gold }, progressText: { color: colors.muted, fontSize: 10, fontWeight: fontWeight.bold }, chevron: { color: colors.muted, fontSize: 24 }, step: { position: 'absolute', top: 10, right: 14, borderRadius: 6, borderWidth: 1, borderColor: '#e8c86a', backgroundColor: '#ffeab8', paddingHorizontal: 7, paddingVertical: 2 }, stepText: { color: '#463100', fontSize: 10, fontWeight: fontWeight.bold },
  stateCard: { minHeight: 150, borderRadius: 12, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.white, alignItems: 'center', justifyContent: 'center', padding: 18 }, stateTitle: { marginTop: 8, color: colors.primary, fontSize: 15, fontWeight: fontWeight.bold, textAlign: 'center' }, stateText: { marginTop: 5, color: colors.muted, fontSize: 11, textAlign: 'center' }, retry: { marginTop: 14, height: 38, borderRadius: 10, backgroundColor: colors.gold, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 18 }, retryText: { color: colors.white, fontSize: 12, fontWeight: fontWeight.bold }, pagination: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 14, paddingVertical: 8 }, pageButton: { width: 36, height: 36, borderRadius: 8, borderWidth: 1, borderColor: colors.borderMid, backgroundColor: colors.white, alignItems: 'center', justifyContent: 'center' }, pageButtonDisabled: { opacity: 0.35 }, pageButtonText: { color: colors.primary, fontSize: 20, lineHeight: 22 }, pageLabel: { color: colors.muted, fontSize: 11, fontWeight: fontWeight.semibold },
  tabs: { height: 84, position: 'relative', backgroundColor: colors.navyDark, flexDirection: 'row', alignItems: 'center', overflow: 'hidden', paddingHorizontal: 16, paddingTop: 10, paddingBottom: 24 }, tab: { flex: 1, alignItems: 'center', justifyContent: 'center', borderRadius: 8, paddingTop: 4, paddingBottom: 2 }, tabSelected: { backgroundColor: 'rgba(0,179,152,0.09)' }, tabCount: { minWidth: 16, height: 16, borderRadius: 8, backgroundColor: '#c4365a', alignItems: 'center', justifyContent: 'center', paddingHorizontal: 4 }, tabCountText: { color: colors.white, fontSize: 9, fontWeight: fontWeight.bold }, tabDot: { width: 16, height: 16, borderRadius: 8, alignItems: 'center', justifyContent: 'center' }, tabDotInner: { width: 4, height: 4, borderRadius: 2, backgroundColor: 'rgba(255,255,255,0.2)' }, tabText: { marginTop: 7, color: 'rgba(255,255,255,0.44)', fontSize: 13, fontWeight: fontWeight.regular }, tabTextSelected: { color: colors.teal, fontWeight: fontWeight.semibold }, tabLine: { marginTop: 5, width: '88%', height: 2, borderRadius: 2, backgroundColor: colors.teal },
  denied: { flex: 1, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 30, backgroundColor: colors.surface }, deniedTitle: { marginTop: 16, color: colors.primary, fontSize: 20, fontWeight: fontWeight.bold }, deniedText: { marginTop: 8, color: colors.muted, fontSize: 13, lineHeight: 19, textAlign: 'center' },
});