...
import {
  countMobileInspectionFilters,
  MobileInspectionFiltersSheet,
} from './MobileInspectionFiltersSheet';
...
function profileBadge(
  ...
  const primary = normalized.find((role) => role.toLowerCase().includes('inspector'))
    ?? normalized[0]
    ?? 'Usuario';
  ...
  const context = [admin, company && area ? `${company} ${area}` : company || area]
    .filter(Boolean)
    .join(' · ');
...
function TypeChip({ value }: { value: string }) {
  ...
  return (
    <View style={[styles.typeChip, checklist ? styles.typeChipChecklist : styles.typeChipFinding]}>
      {checklist
        ? <FontAwesome5 name="clipboard-check" size={9} color="#006153" />
        : <FindingIcon width={12} height={9} />}
      <Text style={[styles.typeChipText, { color: checklist ? '#006153' : '#0d3862' }]}>
...
function CounterRow({ counter }: { counter: Counter }) {
  ...
  const icon = counter.key === 'rejected'
    ? 'times-circle'
    : counter.key === 'open'
      ? 'clock'
      : 'check-circle';
  return (
    <View style={styles.counterRow}>
      <View style={styles.counterLeft}>
        ...
        <Text style={[styles.counterLabel, { color: palette.foreground }]}>
          {counter.count} {counter.count === 1 ? counter.singular : counter.plural}
        </Text>
      </View>
    </View>
  );
      ...
      <View style={styles.cardBody}>
        <View style={styles.cardTop}>
          <Text style={styles.cardId}>
            {row.inspectionNumber.startsWith('#') ? row.inspectionNumber : `#${row.inspectionNumber}`}
          </Text>
        ...
        <View style={styles.cardMeta}>
          ...
          <Text style={styles.cardMetaText}>{row.daysOpen} días</Text>
        </View>
        <View style={styles.cardCounters}>
          {rows.length > 0
            ? rows.map((counter) => <CounterRow key={counter.key} counter={counter} />)
            : <View style={styles.counterRow}><Text style={styles.counterEmpty}>Sin observaciones</Text></View>}
        </View>
        <View style={styles.cardProgress}>
          <View style={styles.cardProgressRail}>
            <View style={[styles.cardProgressFill, { width: `${Math.max(0, Math.min(100, row.closureRate))}%` }]} />
          </View>
          <Text style={styles.cardProgressText}>{Math.round(row.closureRate)}%</Text>
        </View>
      </View>
    </TouchableOpacity>
  );
    ...
    <TouchableOpacity style={styles.historyCard} activeOpacity={0.82} onPress={onPress}>
      ...
      <View style={styles.historyCardBody}>
        ...
        <View style={styles.historyMeta}>
          <FontAwesome5 name="map-marker-alt" size={9} color="#acacac" />
          <Text style={styles.historyMetaText}>{sector} · {row.daysOpen} días</Text>
        </View>
        <View style={styles.historyObservationBar}>
          <FontAwesome5 name="check-circle" size={10} color="#2a5c16" solid />
          <Text style={styles.historyObservationText}>
            {observationCount} {observationCount === 1 ? 'observación' : 'observaciones'}
          </Text>
        </View>
      </View>
    </TouchableOpacity>
      ...
      <TouchableOpacity style={styles.draftBody} onPress={onPress}>
        ...
        <View style={styles.draftCopy}>
          ...
          <View style={styles.progressWrap}>
            <View style={styles.progressRail}><View style={[styles.progressFill, { width: `${safeProgress}%` }]} /></View>
            <Text style={styles.progressText}>{safeProgress}%</Text>
          </View>
        </View>
        <Text style={styles.chevron}>›</Text>
        <View style={styles.step}><Text style={styles.stepText}>Paso {currentStep}/5</Text></View>
      </TouchableOpacity>
    </View>
  );
}
...
function draftDetail(record: import('./manualInspectionDrafts.storage').PersistedManualInspectionDraft): string {
  return [record.draft.sectorName, record.draft.findingCompanyName, relativeTime(record.updatedAt)]
    .filter(Boolean)
    .join(' · ');
}
...
export function MobileInspectionManagementScreen() {
  ...
  const loading = data.table.isLoading
    || (mode === 'management' ? data.managementKpis.isLoading : data.historyKpis.isLoading);
  const refreshing = data.table.isRefetching
    || data.managementKpis.isRefetching
    || data.historyKpis.isRefetching;
  ...
  async function refresh() {
    await Promise.all([
      data.table.refetch(),
      mode === 'management' ? data.managementKpis.refetch() : data.historyKpis.refetch(),
      draftsQuery.refetch(),
    ]);
  }
  ...
  function closeDetail() {
    ...
    if (deepInspectionId || deepFindingId || deepGroup || deepMode || deepNotificationId) {
      ...
      <SafeAreaView style={styles.safe}>
        <View style={styles.denied}>
          ...
          <Text style={styles.deniedText}>
            Tu perfil no cuenta con inspections:read para consultar Gestión de Inspecciones.
          </Text>
        </View>
      </SafeAreaView>
    );
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <View style={styles.screen}>
        <View style={styles.header}>
          ...
          <View style={styles.role}>
            ...
            <Text style={styles.roleText}>{badge}</Text>
          </View>
        </View>
        ...
        <View style={styles.actions}>
          <TouchableOpacity style={styles.filter} onPress={() => setFiltersVisible(true)}>
          ...
          {mode === 'management' && capabilities.create ? (
            <TouchableOpacity style={styles.newButton} onPress={newInspection}>
          ...
          {activeLabels.length > 0 ? (
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.activeFilters}>
              {activeLabels.map((label) => (
                <View key={label} style={styles.activeFilter}><Text style={styles.activeFilterText}>{label}</Text></View>
              ))}
            </ScrollView>
          ) : null}
        </View>
          ...
          {mode === 'management'
            ? drafts.map((draft) => (
                <DraftBox
                  ...
                  onPress={() => { void resumeDraft(draft.draftId); }}
                />
              ))
            : null}
          {loading ? (
            <View style={styles.stateCard}>
              ...
              <Text style={styles.stateTitle}>Cargando {mode === 'history' ? 'historial' : 'gestión'}</Text>
            </View>
          ) : null}
          {!loading && data.table.isError ? (
            <View style={styles.stateCard}>
              <Text style={styles.stateTitle}>No fue posible cargar las inspecciones</Text>
              <Text style={styles.stateText}>Revisa tu conexión o permisos.</Text>
              <TouchableOpacity style={styles.retry} onPress={() => { void refresh(); }}>
                <Text style={styles.retryText}>Reintentar</Text>
              </TouchableOpacity>
            </View>
          ) : null}
          {!loading && !data.table.isError && rows.length === 0 ? (
            <View style={styles.stateCard}>
              <Text style={styles.stateTitle}>Sin inspecciones para mostrar</Text>
              <Text style={styles.stateText}>Ajusta los filtros o sincroniza los datos.</Text>
            </View>
          ) : null}
          {!loading
            ? rows.map((row) => mode === 'history'
              ? <HistoryCard key={row.inspectionId} row={row} onPress={() => setSelectedInspectionId(row.inspectionId)} />
              : <ManagementCard key={row.inspectionId} row={row} onPress={() => setSelectedInspectionId(row.inspectionId)} />)
            : null}
        ...
        <View style={styles.tabs}>
          <FooterGradient />
          <TouchableOpacity
            style={[styles.tab, mode === 'management' && styles.tabSelected]}
            onPress={() => changeMode('management')}
          >
            <View style={styles.tabCount}>
              <Text style={styles.tabCountText}>{data.managementKpis.data?.openInspections ?? 0}</Text>
            </View>
            <Text style={[styles.tabText, mode === 'management' && styles.tabTextSelected]}>
              Gestión de inspecciones
            </Text>
            {mode === 'management' ? <View style={styles.tabLine} /> : null}
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.tab, mode === 'history' && styles.tabSelected]}
      ...
      <MobileInspectionFiltersSheet
        ...
        mode={mode}
        value={filters}
        options={table?.filterOptions ?? { inspectors: [], areas: [], companies: [], types: [], urgencies: [] }}
...
const styles = StyleSheet.create({
  ...
  historyCard: { borderRadius: 12, borderWidth: 1.5, borderColor: colors.border, backgroundColor: colors.white, overflow: 'hidden', shadowColor: '#000', shadowOpacity: 0.06, shadowRadius: 3, shadowOffset: { width: 0, height: 1 }, elevation: 1 },
  historyCardLine: { height: 3, backgroundColor: '#2a5c16' },
  historyCardBody: { paddingHorizontal: 14, paddingTop: 12, paddingBottom: 14 },
  historyCardTitle: { marginTop: 6, color: '#131313', fontSize: 13, lineHeight: 16, fontWeight: fontWeight.bold },
  historyMeta: { marginTop: 3, flexDirection: 'row', alignItems: 'center', gap: 5 },
  historyMetaText: { color: '#646464', fontSize: 11, lineHeight: 14 },
  historyObservationBar: { marginTop: 10, minHeight: 24, borderRadius: 7, backgroundColor: '#e0ffd3', flexDirection: 'row', alignItems: 'center', gap: 6, paddingHorizontal: 8, paddingVertical: 5 },
  historyObservationText: { color: '#2a5c16', fontSize: 11, fontWeight: fontWeight.semibold },
