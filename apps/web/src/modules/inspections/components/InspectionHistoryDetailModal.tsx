...
function EvidencePreview({ title, evidences, afterClosed = false }: { title: string; evidences: InspectionDetailEvidenceResponse[]; afterClosed?: boolean }) {
  ...
  return <div className="flex h-[91px] min-w-0 flex-1 flex-col overflow-hidden rounded-[6px] border border-[#e3e3e3] bg-white p-px"><div className="flex h-[20px] items-center bg-[#001e39] px-[8px] py-[4px]"><p className="text-[9px] font-bold uppercase leading-none text-[rgba(255,255,255,0.7)]">{title}</p></div><div className={`flex min-h-0 flex-1 items-center justify-center overflow-hidden ${afterClosed ? 'bg-[#dafccb]' : 'bg-gradient-to-br from-[#e8f4fd] to-[#c8e6f0]'}`}>{firstEvidence && firstUrl ? <img className="h-full w-full object-cover" src={firstUrl} alt={evidenceLabel(firstEvidence, 0)} /> : <InspectionDetailImageIcon tone={afterClosed ? '#2a5c16' : '#24588b'} />}</div></div>;
}
...
function ResponsiblesSection({ detail }: { detail: InspectionDetailResponse }) {
  ...
  return <GeneralSection icon={<InspectionDetailPersonIcon />} title="Responsables"><GeneralInfoRows rows={[{ label: 'EECC', value: detail.general.companyName ?? '—' }]} /><div className="border-t border-[#e3e3e3]">{responsibles.map((responsible, index) => <div key={responsible.userId} className={`flex items-center gap-[10px] px-[12px] py-[10px] ${index === responsibles.length - 1 ? '' : 'border-b border-[#e3e3e3]'}`}><div className={`flex size-[32px] shrink-0 items-center justify-center rounded-full text-[12px] font-bold leading-none ${avatarColors[index % avatarColors.length]}`}>{initials(responsible.fullName)}</div><div className="min-w-0 flex-1"><p className="truncate text-[12px] font-bold leading-none text-[#131313]">{responsible.fullName}</p><p className="pt-[4px] text-[11px] font-normal leading-none text-[#646464]">{responsible.position ?? 'Sin cargo'}</p></div>{responsible.currentUser ? <span className="inline-flex h-[16px] items-center rounded-[5px] bg-[#c5fff6] px-[7px] text-[10px] font-bold leading-none text-[#00b398]">Tú</span> : null}</div>)}</div></GeneralSection>;
}
