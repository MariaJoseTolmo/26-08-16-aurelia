...
export function AssistantChatStep(props: AssistantChatStepProps) {
  ...
  async function selectSector(sector: SectorResponse, messageId: string) { markResolved(messageId); draft.setSector(sector.id, sector.name); push('user', sector.name); push('typing'); try { const types = await queryClient.fetchQuery({ queryKey: ['inspections', 'assistant-chat', 'inspection-types'], queryFn: getInspectionTypes, staleTime: 300000 }); clearTyping(); const areaName = useNewInspectionDraftStore.getState().areaName; push('bot', <><strong>{areaName} · {sector.name} <InlineCheckIcon /></strong>. ¿Qué tipo de inspección es?</>); push('types', [{ value: InspectionType.ENVIRONMENTAL, label: 'Hallazgo', icon: 'search', inspectionTypeId: typeId(types, InspectionType.ENVIRONMENTAL) }, { value: InspectionType.REGULATORY, label: 'Checklist normativo', icon: 'clipboard-check', inspectionTypeId: typeId(types, InspectionType.REGULATORY) }] as TypeOption[]); } catch { clearTyping(); pushError('No pude cargar tipos.', () => { void selectSector(sector, messageId); }); } }
  ...
  async function askFindingType() { push('typing'); try { const types = await queryClient.fetchQuery({ queryKey: ['inspections', 'assistant-chat', 'finding-types'], queryFn: getInspectionFindingTypes, staleTime: 300000 }); clearTyping(); setStep(1); push('bot', <>Selecciona el <strong>tipo de hallazgo</strong>.</>); push('findingTypes', types); } catch { clearTyping(); pushError('No pude cargar tipos de hallazgo.', () => { void askFindingType(); }); } }
  ...
  async function askPeople(companyId: string) { push('typing'); try { const users = await queryClient.fetchQuery({ queryKey: ['inspections', 'assistant-chat', 'people', companyId], queryFn: () => getCompanyUsers(companyId), staleTime: 300000 }); clearTyping(); if (!users.length) { pushError('No hay personal asociado a esta empresa.'); return; } const suggested = suggestedResponsible(users); push('bot', <>Para <strong>{useNewInspectionDraftStore.getState().findingCompanyName}</strong>, sugiero este personal. Selecciona uno o más:</>); push('people', { users, suggestedUserId: suggested?.id ?? null } as PeopleData); } catch { clearTyping(); pushError('No pude cargar personal.'); } }
