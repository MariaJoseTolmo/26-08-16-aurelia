...
import { AssistantChatStep as AssistantChatStepV3 } from './AssistantChatStepV3';
...
function InlineCheckIcon() {
  return <span className="inline-flex translate-y-[1px] text-[15px] font-bold leading-none text-[#131313]">✓</span>;
}
...
function BotText({ children }: { children: ReactNode }) {
  return <p className="text-[15px] leading-[22px] text-[#131313] [&_strong]:font-bold [&_strong]:text-[#131313]">{children}</p>;
}
...
function PhotoWidget({ resolved, receipt, onCapture }: { resolved: boolean; receipt?: PhotoReceipt; onCapture: (asset: NewInspectionPickedAsset) => void }) {
  function onFile(event: ChangeEvent<HTMLInputElement>) { const file = event.target.files?.[0]; if (!file) return; onCapture(fileAsset(file)); event.target.value = ''; }
...
function CompanySearchSheet({ visible, companies, selectedId, onClose, onSelect }: { visible: boolean; companies: CompanyResponse[]; selectedId?: string | null; onClose: () => void; onSelect: (company: CompanyResponse) => void }) {
  const [query, setQuery] = useState('');
  const filteredCompanies = useMemo(() => { const value = normalizeText(query); if (!value) return companies; return companies.filter((company) => normalizeText(company.name).includes(value)); }, [companies, query]);
  useEffect(() => { if (visible) setQuery(''); }, [visible]);
  if (!visible) return null;
  return <div className="fixed bottom-[16px] right-[20px] top-[16px] z-[1100] flex w-[360px] max-w-[calc(100vw-40px)] items-end overflow-hidden rounded-[22px] bg-[rgba(19,19,19,0.75)]" onClick={onClose}><div className="flex h-[705px] max-h-[88%] w-full flex-col rounded-t-[16px] bg-white px-[14px] pb-[24px] pt-[12px] shadow-[0_-12px_32px_rgba(0,0,0,0.22)]" onClick={(event) => event.stopPropagation()}><div className="flex w-full flex-col items-center pt-[10px]"><div className="h-[4px] w-[40px] rounded-[2px] bg-[#D1D1D1]" /></div><div className="mt-[24px] grid gap-[12px]"><p className="text-[18px] font-bold leading-[21.6px] text-[#131313]">Seleccione la empresa</p><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Ingrese nombre de la empresa" className="h-[50px] w-full rounded-[10px] border-[1.5px] border-[#24588B] bg-[#F6FAFF] px-[15.5px] py-[15px] text-[13px] leading-[19.5px] text-[#131313] outline-none placeholder:text-[#131313]" /></div><div className="mt-[12px] min-h-0 flex-1 overflow-y-auto rounded-[12px] bg-white p-[8px]">{filteredCompanies.length === 0 ? <p className="px-[8px] py-[12px] text-[14px] leading-[22.7px] tracking-[0.28px] text-[#646464]">No hay empresas disponibles</p> : null}{filteredCompanies.map((company) => <button key={company.id} type="button" onClick={() => onSelect(company)} className={`flex min-h-[46.7px] w-full items-center rounded-[8px] px-[8px] py-[12px] text-left ${company.id === selectedId ? 'bg-[#F6FAFF]' : 'bg-white'}`}><span className="min-w-0 flex-1 text-[14px] font-normal leading-[22.7px] tracking-[0.28px] text-[#131313]">{company.name}</span></button>)}</div></div></div>;
}

function CompanyPicker({ companies, selectedId, resolved, onSelect }: { companies: CompanyResponse[]; selectedId?: string | null; resolved: boolean; onSelect: (company: CompanyResponse) => void }) {
  const [open, setOpen] = useState(!resolved);
  const selected = companies.find((company) => company.id === selectedId) ?? null;
  useEffect(() => { if (!resolved) setOpen(true); }, [resolved]);
  return <><div className="mb-[10px] ml-[33px] mr-[12px] rounded-[12px] border border-[#E3E3E3] bg-white p-[12px]"><p className="text-[12px] font-bold text-[#131313]">Empresa responsable</p><button type="button" disabled={resolved} onClick={() => setOpen(true)} className="mt-[8px] flex h-[50px] w-full items-center justify-between rounded-[10px] border-[1.5px] border-[#24588B] bg-[#F6FAFF] px-[15.5px] py-[15px] text-left text-[13px] leading-[19.5px] text-[#131313] disabled:opacity-80"><span className="truncate">{selected?.name ?? 'Seleccione la empresa'}</span><span className="text-[16px] leading-none text-[#131313]">⌄</span></button></div><CompanySearchSheet visible={open && !resolved} companies={companies} selectedId={selectedId} onClose={() => setOpen(false)} onSelect={(company) => { setOpen(false); onSelect(company); }} /></>;
}
...
function ObservationSavedCard({ observation, index, onRemove }: { observation: NewInspectionFindingObservationDraft | null; index: number; onRemove: () => void }) {
  ...
  return <div className="assistant-observation-saved-card mb-[10px] mx-[12px] w-[calc(100%-24px)] rounded-[10px] border border-[#E3E3E3] bg-white px-[12px] py-[9px] shadow-[0_1px_2px_rgba(0,0,0,0.04)]"><div className="flex items-start gap-[8px]"><span className="shrink-0 rounded-[5px] bg-[#E6F3FF] px-[7px] py-[2px] text-[10px] font-bold leading-[14px] text-[#0D3862]">Obs. {index}</span><div className="min-w-0 flex-1"><p className="truncate text-[12px] font-semibold leading-[17px] text-[#131313]">{observation.detectedCondition}</p><div className="mt-[3px] flex flex-wrap items-center gap-[4px]"><span className="rounded-[4px] bg-[#FFEAB8] px-[6px] py-[2px] text-[9px] font-bold leading-[12px] text-[#463100]">{observation.severityLabel ?? 'Manual'} · {parseSlaDays(observation.severityClosureTimeLabel, 7)}d</span><span className="rounded-[4px] bg-[#F4F6F9] px-[6px] py-[2px] text-[9px] font-bold leading-[12px] text-[#646464]">Manual</span>{observation.evidence ? <span className="inline-flex items-center gap-[1px]"><EvidenceIcon /><CheckMiniIcon /></span> : null}</div></div><button type="button" aria-label="Eliminar observación" onClick={onRemove} className="flex h-[24px] w-[24px] shrink-0 items-center justify-center rounded-[5px] border border-[#E3E3E3] bg-[#F4F6F9] text-[#646464] transition hover:border-[#CC315F] hover:bg-[#FFD4E0] hover:text-[#7A0E23]"><TrashIcon /></button></div></div>;
}
...
function SummaryCard({ responsibleUsers, errorMessage }: { responsibleUsers: UserResponse[]; errorMessage: string | null }) {
  const state = useNewInspectionDraftStore();
  const saved = state.findingObservations.filter((item) => item.saved);
  const responsibleNames = responsibleUsers.length > 0 ? responsibleUsers.map((item) => item.fullName).join(' · ') : state.findingResponsibleIds.length > 0 ? `${state.findingResponsibleIds.length} responsables` : '—';
  return <div className="mb-[10px] ml-[33px] mr-[12px] grid gap-[10px]"><div className="overflow-hidden rounded-[10px] border border-[#E3E3E3] bg-white"><div className="flex items-center justify-between bg-[#001E39] px-[12px] py-[7px]"><p className="text-[11px] font-bold text-white">Datos generales</p><span className="rounded-[4px] bg-[#A7F3E7] px-[7px] py-[2px] text-[9px] font-bold text-white">{state.inspectionTypeLabel || 'Hallazgo'}</span></div><p className="border-b border-[#E3E3E3] px-[12px] py-[7px] text-[11px]"><span className="text-[#646464]">Inspector</span><b className="float-right max-w-[190px] truncate text-right">{state.inspectorName}</b></p><p className="border-b border-[#E3E3E3] px-[12px] py-[7px] text-[11px]"><span className="text-[#646464]">Área · Sector</span><b className="float-right max-w-[190px] truncate text-right">{[state.areaName, state.sectorName].filter(Boolean).join(' · ')}</b></p><p className="border-b border-[#E3E3E3] px-[12px] py-[7px] text-[11px]"><span className="text-[#646464]">Empresa EECC</span><b className="float-right max-w-[190px] truncate text-right">{state.findingCompanyName ?? '—'}</b></p><p className="px-[12px] py-[7px] text-[11px]"><span className="text-[#646464]">Responsables</span><b className="float-right max-w-[190px] truncate text-right">{responsibleNames}</b></p></div><div className="overflow-hidden rounded-[10px] border border-[#E3E3E3] bg-white"><div className="bg-[#001E39] px-[12px] py-[7px]"><p className="text-[11px] font-bold text-white">{saved.length} Observaciones</p></div><div className="grid gap-[6px] p-[12px]">{saved.map((item, index) => <div key={item.id} className="rounded-[8px] border border-[#E3E3E3] p-[10px]"><div className="mb-[6px] flex items-center justify-between"><span className="rounded-[5px] bg-[#E6F3FF] px-[7px] py-[2px] text-[10px] font-bold text-[#0D3862]">Obs. {index + 1}</span><span className="flex items-center gap-[5px]"><span className="rounded-[4px] bg-[#FFEAB8] px-[7px] py-[2px] text-[9px] font-bold text-[#463100]">{item.severityLabel ?? 'Manual'}</span><span className="rounded-[4px] bg-[#FDF3E3] px-[6px] py-[2px] text-[9px] font-bold text-[#8E6E3E]">IA</span></span></div><p className="text-[12px] font-medium leading-[17px] text-[#131313]">{item.detectedCondition}</p><p className="mt-[3px] text-[10px] leading-[14px] text-[#646464]">SLA: {parseSlaDays(item.severityClosureTimeLabel, 7)} días</p></div>)}</div></div>{errorMessage ? <p className="rounded-[8px] bg-[#FFD4E0] px-[10px] py-[8px] text-[12px] font-semibold text-[#7A0E23]">{errorMessage}</p> : null}</div>;
}

function BottomSaveBar({ onSave, saving }: { onSave: () => void; saving: boolean }) {
  return <div className="border-t border-[#E3E3E3] bg-white px-[14px] pb-[10px] pt-[12px]"><button type="button" onClick={onSave} disabled={saving} className="flex h-[48px] w-full items-center justify-center gap-[10px] rounded-[12px] bg-[#35A137] text-[15px] font-bold text-white shadow-[0_2px_8px_rgba(53,161,55,0.18)] disabled:opacity-70">{saving ? 'Guardando…' : '✓ Guardar inspección'}</button><div className="mx-auto mb-[4px] mt-[14px] h-[4px] w-[120px] rounded-[2px] bg-[#D1D1D1]" /></div>;
}
...
function renderBotContent(data: unknown) {
  if (typeof data === 'string') return <BotText>{data}</BotText>;
  return <BotText>{data as ReactNode}</BotText>;
}
...
export function AssistantChatStep(props: AssistantChatStepProps) {
  ...
  const [selectedResponsibleUsers, setSelectedResponsibleUsers] = useState<UserResponse[]>([]);
  ...
  function push(t: MsgType, data?: unknown) { if (t === 'typing') typingShownAt.current = Date.now(); sequence.current += 1; const message = { id: String(sequence.current), t, data }; setMessages((prev) => [...prev, message]); return message.id; }
  async function clearTyping() { const remaining = Math.max(0, 500 - (Date.now() - typingShownAt.current)); if (remaining > 0) await sleep(remaining); setMessages((prev) => prev.filter((message) => message.t !== 'typing')); }
  ...
  if (fallbackToV3) return <AssistantChatStepV3 {...props} resumeFromDraft />;

  function resetChatState() { retries.current.clear(); setMessages([]); setResolvedMessages(new Set()); setConfirmedPeople(new Set()); setSelectedResponsibleUsers([]); setPhotoReceiptByMessageId({}); setWaiting(null); setInputValue(''); setStep(0); }
  ...
  async function resumeChat() { resetChatState(); const state = useNewInspectionDraftStore.getState(); if (state.inspectionType !== InspectionType.ENVIRONMENTAL && state.inspectionTypeLabel) { setFallbackToV3(true); return; } push('bot', 'Retomé tu borrador. Continuemos.'); if (!state.areaId) { await beginAreaSelection(); return; } if (!state.sectorId) { await beginSectorSelection(state.areaId); return; } if (!state.locationCaptured) { push('bot', <>Capturemos la <strong>ubicación obligatoria</strong>.</>); push('loc'); return; } await askFindingType(); }
  async function beginAreaSelection() { push('typing'); try { const areas = await queryClient.fetchQuery({ queryKey: ['inspections', 'assistant-chat', 'areas'], queryFn: getOrganizationAreas, staleTime: 300000 }); await clearTyping(); const inspectorName = useNewInspectionDraftStore.getState().inspectorName; push('bot', <>¡Hola, <strong>{inspectorName}</strong>! 👋 Soy AurelIA. Voy a ayudarte a registrar esta inspección de forma rápida. ¿En qué <strong>área</strong> estás hoy?</>); push('areas', areas); } catch { await clearTyping(); pushError('No pude cargar áreas.', () => { void beginAreaSelection(); }); } }
  async function beginSectorSelection(areaId: string) { push('typing'); try { const sectors = await queryClient.fetchQuery({ queryKey: ['inspections', 'assistant-chat', 'sectors', areaId], queryFn: () => getOrganizationSectors(areaId), staleTime: 300000 }); await clearTyping(); const areaName = useNewInspectionDraftStore.getState().areaName; push('bot', <>Perfecto, <strong>{areaName}</strong> <InlineCheckIcon />. Ahora el sector — ¿en cuál específicamente?</>); push('sectors', sectors); } catch { await clearTyping(); pushError('No pude cargar sectores.', () => { void beginSectorSelection(areaId); }); } }
  ...
  async function selectSector(sector: SectorResponse, messageId: string) { markResolved(messageId); draft.setSector(sector.id, sector.name); push('user', sector.name); push('typing'); try { const types = await queryClient.fetchQuery({ queryKey: ['inspections', 'assistant-chat', 'inspection-types'], queryFn: getInspectionTypes, staleTime: 300000 }); await clearTyping(); const areaName = useNewInspectionDraftStore.getState().areaName; push('bot', <><strong>{areaName} · {sector.name} <InlineCheckIcon /></strong>. ¿Qué tipo de inspección es?</>); push('types', [{ value: InspectionType.ENVIRONMENTAL, label: 'Hallazgo', icon: 'search', inspectionTypeId: typeId(types, InspectionType.ENVIRONMENTAL) }, { value: InspectionType.REGULATORY, label: 'Checklist normativo', icon: 'clipboard-check', inspectionTypeId: typeId(types, InspectionType.REGULATORY) }] as TypeOption[]); } catch { await clearTyping(); pushError('No pude cargar tipos.', () => { void selectSector(sector, messageId); }); } }
  async function selectInspectionType(value: InspectionType, inspectionTypeId: string | null, label: string, messageId: string) { markResolved(messageId); draft.setInspectionType(value, label); push('user', label); if (!inspectionTypeId) { pushError('No se encontró este tipo en catálogo.'); return; } if (value !== InspectionType.ENVIRONMENTAL) { setFallbackToV3(true); return; } await sleep(200); push('bot', <>Selecciona la <strong>fecha de inspección</strong>.</>); push('date'); }
  async function selectInspectionDate(value: string, messageId: string) { if (!value || resolvedMessages.has(messageId)) return; markResolved(messageId); draft.setInspectionDate(value); push('user', value); await sleep(200); push('bot', <>Capturemos la <strong>ubicación obligatoria</strong>.</>); push('loc'); }
  ...
  async function askFindingType() { push('typing'); try { const types = await queryClient.fetchQuery({ queryKey: ['inspections', 'assistant-chat', 'finding-types'], queryFn: getInspectionFindingTypes, staleTime: 300000 }); await clearTyping(); setStep(1); push('bot', <>Selecciona el <strong>tipo de hallazgo</strong>.</>); push('findingTypes', types); } catch { await clearTyping(); pushError('No pude cargar tipos de hallazgo.', () => { void askFindingType(); }); } }
  ...
  async function startFindingObservation() { setStep(1); const id = draft.addFindingObservation(); await sleep(200); const state = useNewInspectionDraftStore.getState(); push('bot', <>Cuéntame la condición subestándar que detectaste en <strong>{state.areaName} · {state.sectorName}</strong>.</>); setWaiting(`finding-cond:${id}`); }
  ...
  async function askCriticality(observationId: string) { setStep(2); push('typing'); try { const severities = await queryClient.fetchQuery({ queryKey: ['inspections', 'assistant-chat', 'finding-severities'], queryFn: getInspectionFindingSeverities, staleTime: 300000 }); await clearTyping(); push('bot', 'Definamos la criticidad del hallazgo.'); push('criticality', { observationId, severities } as CriticalityData); } catch { await clearTyping(); pushError('No pude cargar criticidades.'); } }
  ...
  async function askCompany() { push('typing'); try { const companies = await queryClient.fetchQuery({ queryKey: ['inspections', 'assistant-chat', 'companies'], queryFn: getResponsibleCompanies, staleTime: 300000 }); await clearTyping(); if (!companies.length) { pushError('No hay empresas disponibles.'); return; } let suggestion = companies[0].name; let fallback = true; try { const state = useNewInspectionDraftStore.getState(); const response = await suggestCompany({ area: state.areaName ?? '', sector: state.sectorName ?? '', availableCompanies: companies.map((company) => company.name) }); suggestion = response.suggestion; fallback = response.fallback; } catch { fallback = true; } const company = matchSuggestedCompany(companies, suggestion); if (!company) { pushError('No pude sugerir empresa.'); return; } const state = useNewInspectionDraftStore.getState(); setStep(4); push('bot', <>Basándome en el historial de <strong>{state.areaName} · {state.sectorName}</strong>, te propongo:</>); push('companySuggestion', { company, companies, fallback } as CompanySuggestionData); } catch { await clearTyping(); pushError('No pude cargar empresas.'); } }
  ...
  async function askPeople(companyId: string) { push('typing'); try { const users = await queryClient.fetchQuery({ queryKey: ['inspections', 'assistant-chat', 'people', companyId], queryFn: () => getCompanyUsers(companyId), staleTime: 300000 }); await clearTyping(); if (!users.length) { pushError('No hay personal asociado a esta empresa.'); return; } const suggested = suggestedResponsible(users); push('bot', <>Para <strong>{useNewInspectionDraftStore.getState().findingCompanyName}</strong>, sugiero este personal. Selecciona uno o más:</>); push('people', { users, suggestedUserId: suggested?.id ?? null } as PeopleData); } catch { await clearTyping(); pushError('No pude cargar personal.'); } }
  async function confirmPeople(users: UserResponse[], messageId: string) { setConfirmedPeople((prev) => new Set(prev).add(messageId)); if (!users.length) return; setSelectedResponsibleUsers(users); draft.setFindingResponsibles(users.map((item) => item.id)); push('user', `✓ Personal: ${users.map((item) => item.fullName).join(', ')}`); await showSummary(); }
  async function showSummary() { setStep(5); await sleep(200); push('bot', <>¡Listo! Revisa el <strong>resumen</strong> antes de guardar:</>); push('summary'); }
  ...
  function renderMessage(message: Msg) {
    ...
    if (message.t === 'bot') return <BotBubble key={message.id}>{renderBotContent(message.data)}</BotBubble>;
    ...
    if (message.t === 'companies') { const companies = message.data as CompanyResponse[]; return <CompanyPicker key={message.id} companies={companies} selectedId={draft.findingCompanyId} resolved={resolved} onSelect={(company) => { if (!resolved) void selectCompany(company, message.id); }} />; }
